    ...
    btnContacts(new QPushButton())
{
    ...
    // Connect
    connect(ui->pushLeft, &QPushButton::clicked, [this](){onCORRSelected(true);});
    connect(ui->pushRight,  &QPushButton::clicked, [this](){onCORRSelected(false);});
...
void SendWidget::refreshAmounts()
{
    ...
    bool isZcorr = ui->pushRight->isChecked();
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();

    ui->labelAmountSend->setText(GUIUtil::formatBalance(total, nDisplayUnit, isZcorr));
...
void SendWidget::showHideCheckBoxDelegations()
{
    ...
    // coincontrol is not selected, and we are trying to spend CORR (not zCORR)
    const bool isZcorr = ui->pushRight->isChecked();
    const bool isCControl = CoinControlDialog::coinControl->HasSelected();
    const bool hasDel = cachedDelegatedBalance > 0;

    const bool showCheckBox = !isZcorr && !isCControl && hasDel;
    ...
    if (showCheckBox)
        ui->checkBoxDelegations->setToolTip(
                tr("Possibly spend coins delegated for cold-staking (currently available: %1").arg(
                        GUIUtil::formatBalance(cachedDelegatedBalance, nDisplayUnit, isZcorr))
        );
}
...
void SendWidget::onSendClicked()
{
    ...
    bool sendCorr = ui->pushLeft->isChecked();
    ...
    if ((sendCorr) ? send(recipients) : sendZcorr(recipients)) {
...
bool SendWidget::sendZcorr(QList<SendCoinsRecipient> recipients)
{
    if (!walletModel || !walletModel->getOptionsModel())
        return false;

    if (sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        Q_EMIT message(tr("Spend Zerocoin"), tr("zCORR is currently undergoing maintenance."), CClientUIInterface::MSG_ERROR);
        return false;
    }
    ...
    // use mints from zCORR selector if applicable
    std::vector<CMintMeta> vMintsToFetch;
    std::vector<CZerocoinMint> vMintsSelected;
    if (!ZCorrControlDialog::setSelectedMints.empty()) {
        vMintsToFetch = ZCorrControlDialog::GetSelectedMints();
    ...
    if (walletModel->sendZcorr(
        ...
        inform(tr("zCORR transaction sent!"));
        ZCorrControlDialog::setSelectedMints.clear();
        clearAll(false);
        return true;
    } else {
        QString body;
        if (receipt.GetStatus() == ZCORR_SPEND_V1_SEC_LEVEL) {
            body = tr("Version 1 zCORR require a security level of 100 to successfully spend.");
        } else {
            int nNeededSpends = receipt.GetNeededSpends(); // Number of spends we would need for this transaction
            const int nMaxSpends = Params().GetConsensus().ZC_MaxSpendsPerTx; // Maximum possible spends for one zCORR transaction
        ...
        Q_EMIT message("zCORR transaction failed", body, CClientUIInterface::MSG_ERROR);
        return false;
    }
}
...
void SendWidget::onCoinControlClicked()
{
    if (isCORR) {
        ...
        if (walletModel->getZerocoinBalance() > 0) {
            ZCorrControlDialog *zCorrControl = new ZCorrControlDialog(this);
            zCorrControl->setModel(walletModel);
            zCorrControl->exec();
            ui->btnCoinControl->setActive(!ZCorrControlDialog::setSelectedMints.empty());
            zCorrControl->deleteLater();
        } else {
            inform(tr("You don't have any zCORR in your balance to select."));
        }
    }
}
...
void SendWidget::onCORRSelected(bool _isCORR)
{
    isCORR = _isCORR;
    setCssProperty(coinIcon, _isCORR ? "coin-icon-corr" : "coin-icon-zcorr");
    refreshView();
