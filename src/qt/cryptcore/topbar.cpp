    ...
    ui(new Ui::TopBar)
{
    ...
    std::initializer_list<QWidget*> lblTitles = {ui->labelTitle1, ui->labelTitleAvailablezCorr, ui->labelTitle3, ui->labelTitle4, ui->labelTitlePendingzCorr, ui->labelTitleImmaturezCorr};
    ...
    setCssProperty({ui->labelAmountTopCorr, ui->labelAmountTopzCorr}, "amount-small-topbar");
    setCssProperty({ui->labelAmountCorr, ui->labelAvailablezCorr}, "amount-topbar");
    setCssProperty({ui->labelPendingCorr, ui->labelPendingzCorr, ui->labelImmatureCorr, ui->labelImmaturezCorr}, "amount-small-topbar");

    ui->labelTitleAvailablezCorr->setVisible(false);
    ui->labelAmountTopzCorr->setVisible(false);
    ui->labelAvailablezCorr->setVisible(false);
    ui->labelTitlePendingzCorr->setVisible(false);
    ui->labelPendingzCorr->setVisible(false);
    ui->labelTitleImmaturezCorr->setVisible(false);
    ui->labelImmaturezCorr->setVisible(false);
                            ...
                            const CAmount& delegatedBalance, const CAmount& coldStakedBalance)
{
    ...
    // CORR Total
    CAmount corrAvailableBalance = balance;
    // zCORR Balance
    CAmount matureZerocoinBalance = zerocoinBalance - unconfirmedZerocoinBalance - immatureZerocoinBalance;

    // Set
    QString totalCorr = GUIUtil::formatBalance(corrAvailableBalance, nDisplayUnit);
    QString totalzCorr = GUIUtil::formatBalance(matureZerocoinBalance, nDisplayUnit, true);
    ...
    // Update display state and/or values for zCORR balances as necessary
    bool fHaveZerocoins = zerocoinBalance > 0;

    // Set visibility of zCORR label titles/values
    ui->typeSpacerTop->setVisible(fHaveZerocoins);
    ui->typeSpacerExpanded->setVisible(fHaveZerocoins);
    ui->labelAmountTopzCorr->setVisible(fHaveZerocoins);
    ui->zerocoinBalances->setVisible(fHaveZerocoins);

    // Top
    ui->labelAmountTopzCorr->setText(totalzCorr);
    // Expanded
    ui->labelAvailablezCorr->setText(totalzCorr);
    ui->labelPendingzCorr->setText(GUIUtil::formatBalance(unconfirmedZerocoinBalance, nDisplayUnit, true));
    ui->labelImmaturezCorr->setText(GUIUtil::formatBalance(immatureZerocoinBalance, nDisplayUnit, true));
}
