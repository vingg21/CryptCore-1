    ...
    ui(new Ui::DashboardWidget)
{
    ...
    setCssProperty(ui->labelSquarezCorr, "square-chart-zcorr");
    setCssProperty(ui->labelCorr, "text-chart-corr");
    setCssProperty(ui->labelZcorr, "text-chart-zcorr");
    ui->labelZcorr->setVisible(false);
    ui->labelSquarezCorr->setVisible(false);
    ...
    ui->labelAmountZcorr->setVisible(false);
    setCssProperty(ui->labelAmountCorr, "text-stake-corr-disable");
    setCssProperty(ui->labelAmountZcorr, "text-stake-zcorr-disable");
...
void DashboardWidget::loadWalletModel()
{
    ...
    // update the display unit, to not use the default ("CORR")
    updateDisplayUnit();
}
...
const QMap<int, std::pair<qint64, qint64>> DashboardWidget::getAmountBy()
{
    ...
    for (int i = 0; i < size; ++i) {
        ...
        bool isCorr = modelIndex.data(TransactionTableModel::TypeRole).toInt() != TransactionRecord::StakeZCORR;
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    chartData->amountsByCache = getAmountBy(); // pair CORR, zCORR
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            zcorr = (pair.second != 0) ? pair.second / 100000000 : 0;
            chartData->totalCorr += pair.first;
            chartData->totalZcorr += pair.second;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesCorr.append(corr);
        chartData->valueszCorr.append(zcorr);

        int max = std::max(corr, zcorr);
...
void DashboardWidget::onChartRefreshed()
{
    ...
    set1->append(chartData->valueszCorr);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalCorr > 0 || chartData->totalZcorr > 0) {
        setCssProperty(ui->labelAmountCorr, "text-stake-corr");
        setCssProperty(ui->labelAmountZcorr, "text-stake-zcorr");
    } else {
        setCssProperty(ui->labelAmountCorr, "text-stake-corr-disable");
        setCssProperty(ui->labelAmountZcorr, "text-stake-zcorr-disable");
    }
    forceUpdateStyle({ui->labelAmountCorr, ui->labelAmountZcorr});
    ui->labelAmountCorr->setText(GUIUtil::formatBalance(chartData->totalCorr, nDisplayUnit));
    ui->labelAmountZcorr->setText(GUIUtil::formatBalance(chartData->totalZcorr, nDisplayUnit, true));
