...
CzCORRTracker::CzCORRTracker(CWallet* parent)
{
    this->wallet = parent;
...
CzCORRTracker::~CzCORRTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzCORRTracker::Init()
{
...
bool CzCORRTracker::Archive(CMintMeta& meta)
{
...
bool CzCORRTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(wallet->strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzCORRTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzCORRTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzCORRTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzCORRTracker::GetSerialHashes()
{
...
CAmount CzCORRTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzCORRTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzCORRTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzCORRTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzCORRTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzCORRTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzCORRTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzCORRTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzCORRTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzCORRTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(wallet->strWalletFile);
...
void CzCORRTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzCORRWallet* zCORRWallet)
{
    bool iszCORRWalletInitialized = (nullptr != zCORRWallet);
    CMintMeta meta;
    ...
    if (!iszCORRWalletInitialized)
        zCORRWallet = new CzCORRWallet(wallet);
    meta.isSeedCorrect = zCORRWallet->CheckSeed(dMint);
    if (!iszCORRWalletInitialized)
        delete zCORRWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzCORRTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzCORRTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzCORRTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzCORRTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzCORRTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzCORRTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(wallet->strWalletFile);
    if (fUpdateStatus) {
        ...
        LogPrint(BCLog::LEGACYZC, "%s: added %d dzcorr from DB\n", __func__, listDeterministicDB.size());
    }
