...
bool CLegacyZCorrStake::InitFromTxIn(const CTxIn& txin)
{
    // Construct the stakeinput object
    if (!txin.IsZerocoinSpend())
        return error("%s: unable to initialize CLegacyZCorrStake from non zc-spend");
    ...
    *this = CLegacyZCorrStake(spend);

    // Find the pindex with the accumulator checksum
    if (!GetIndexFrom())
        return error("%s : Failed to find the block index for zcorr stake origin", __func__);

    // All good
    return true;
}

CLegacyZCorrStake::CLegacyZCorrStake(const libzerocoin::CoinSpend& spend)
{
...
CBlockIndex* CLegacyZCorrStake::GetIndexFrom()
{
...
CAmount CLegacyZCorrStake::GetValue() const
{
    return denom * COIN;
}

CDataStream CLegacyZCorrStake::GetUniqueness() const
{
...
bool CLegacyZCorrStake::ContextCheck(int nHeight, uint32_t nTime)
{
    const Consensus::Params& consensus = Params().GetConsensus();
    if (nHeight < consensus.height_start_ZC_SerialsV2 || nHeight >= consensus.height_last_ZC_AccumCheckpoint)
        return error("%s : zCORR stake block: height %d outside range", __func__, nHeight);
