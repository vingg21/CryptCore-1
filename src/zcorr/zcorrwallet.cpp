...
CzCORRWallet::CzCORRWallet(CWallet* parent)
{
    this->wallet = parent;
    ...
    //Check for old db version of storing zcorr seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZCORRSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (wallet->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZCORRSeed_deprecated()) {
                    LogPrintf("%s: Updated zCORR seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zcorr seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fRegtest && fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zcorr wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzCORRWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZCORRCount(nCountLastUsed);
    else if (!walletdb.ReadZCORRCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzCORRWallet::Lock()
{
    seedMaster.SetNull();
}

void CzCORRWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzCORRWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZCORR(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzCORRWallet::LoadMintPoolFromDB()
{
...
void CzCORRWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzCORRWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzCORRWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (std::pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (wallet->zcorrTracker->HasPubcoinHash(pMint.first)) {
...
bool CzCORRWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZCORR(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zcorrTracker which also adds to database
    wallet->zcorrTracker->Add(dMint, true);
...
void CzCORRWallet::SeedToZCORR(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
    libzerocoin::ZerocoinParams* params = Params().GetConsensus().Zerocoin_Params(false);
...
uint512 CzCORRWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzCORRWallet::GenerateDeterministicZCORR(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzCORRWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzCORRWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzCORRWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
