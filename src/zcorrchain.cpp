...
{
    CDataStream serializedCoinSpend = ZCORRModule::ScriptSigToSerializedSpend(txin.scriptSig);
...
bool UpdateZCORRSupplyConnect(const CBlock& block, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    //Add mints to zCORR supply (mints are forever disabled after last checkpoint)
    ...
    //Remove spends from zCORR supply
    ...
    // A one-time event where only the zCORR supply was off (due to serial duplication off-chain on main net)
...
bool UpdateZCORRSupplyDisconnect(const CBlock& block, CBlockIndex* pindex)
{
    ...
    // Re-add spends to zCORR supply
    std::list<libzerocoin::CoinDenomination> listDenomsSpent = ZerocoinSpendListFromBlock(block, true);
    for (const libzerocoin::CoinDenomination& denom : listDenomsSpent) {
        mapZerocoinSupply.at(denom)++;
    }

    // Remove mints from zCORR supply (mints are forever disabled after last checkpoint)
    if (pindex->nHeight < consensus.height_last_ZC_AccumCheckpoint) {
        std::list<CZerocoinMint> listMints;
        std::set<uint256> setAddedToWallet;
        BlockToZerocoinMintList(block, listMints, true);
