...
The build script we'll be using is contained in the CRYPTCORE github repository ([contrib/gitian-build.py](https://github.com/Cryptcore/cryptcore/blob/master/contrib/gitian-build.py)). Since this is a completely fresh environment, we haven't yet cloned the CRYPTCORE repository and will need to fetch this script with the following commands:

```bash
curl -L -O https://raw.githubusercontent.com/Cryptcore/CRYPTCORE/master/contrib/gitian-build.py
